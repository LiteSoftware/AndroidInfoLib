...
class GetWirelessInformationCommand(
    private val api: AndroidInfoApi
) : CommandInterface<Unit, WirelessInformation> {

    override fun invoke(param: Unit) = MutableWirelessInformation().also {
        GetCloudWirelessInformationCommand(api).invoke(it)
    }.build()
}