...
class GetDisplayInformationCommand(
    val context: Context,
    val api: AndroidInfoApi
) : CommandInterface<Unit, DisplayInformation> {

    override fun invoke(param: Unit) = MutableDisplayInformation().also {
        GetLocalDisplayInformationCommand(context).invoke(it)
        GetCloudDisplayInformation(api).invoke(it)
    }.build()
}