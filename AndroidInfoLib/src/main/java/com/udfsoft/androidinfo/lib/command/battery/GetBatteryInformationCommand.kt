...
class GetBatteryInformationCommand(
    private val api: AndroidInfoApi
) : CommandInterface<Unit, BatteryInformation> {

    override fun invoke(param: Unit) = MutableBatteryInformation().also {
        GetCloudBatteryInformationCommand(api).invoke(it)
    }.build()
}