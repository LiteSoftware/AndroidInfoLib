...
class GetRearCameraInformationCommand(
    private val api: AndroidInfoApi
) : CommandInterface<Unit, RearCameraInformation> {

    override fun invoke(param: Unit) = MutableRearCameraInformation().also {
        GetCloudRearCameraInformation(api).invoke(it)
    }.build()
}