...
class GetCodecsInformationCommand(
    private val api: AndroidInfoApi
) : CommandInterface<Unit, CodecsInformation> {

    override fun invoke(param: Unit) = MutableCodecsInformation().also {
        GetCloudCodecsInformationCommand(api).invoke(it)
    }.build()
}