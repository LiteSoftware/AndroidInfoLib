...
class GetSARInformationCommand(
    private val api: AndroidInfoApi
) : CommandInterface<Unit, SARInformation> {

    override fun invoke(param: Unit) = MutableSARInformation().also {
        GetCloudSARInformationCommand(api).invoke(it)
    }.build()
}